...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (n-- > 0) {
            ...
            if ("Car".equals(vehicle)) {
                tryIt(vehicles, action, value, vehicle);
    ...
    private static void tryIt(Map<String, Vehicle> vehicles, String action, Double value, String mapKey) {
        if ("Refuel".equals(action)) {
            try {
                vehicles.get(mapKey).refuel(value);
            } catch (IllegalArgumentException e) {
                System.out.println(e.getMessage());
            }
        } else {
            try {
                vehicles.get(mapKey).drive(value);
            } catch (IllegalArgumentException e) {
                System.out.println(e.getMessage());
            }
        }
